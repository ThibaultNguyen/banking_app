...
public interface BudgetClient {
    ...
    @POST(BUDGET_RECURRING)
    @Headers(CONTENT_TYPE_APPLICATION_JSON)
    Mono<BudgetResponseClientDto> createRecurring(@Header(AUTHORIZATION) String customerBearer,
                                                  @Body BudgetClientDto budgetClientDto);
    ...
    @PUT(BUDGET_ID)
    @Headers(CONTENT_TYPE_APPLICATION_JSON)
    Mono<BudgetResponseClientDto> update(@Header(AUTHORIZATION) String customerBearer,
                                         @Path(value = "id") String budgetId, @Body BudgetClientDto budgetClientDto);

    @GET(BUDGET_DETAILS)
    Mono<BudgetDetailsResponseClientDto> findById(@Header(AUTHORIZATION) String customerBearer,
                                                  @Path(value = "id") String budgetId,
                                                  @Query(value = "start") Long start,
                                                  @Query(value = "end") Long end);

    @GET(BUDGET_TRANSACTIONS)
    Mono<BudgetTransactionsResponseClientDto> findTransactions(@Header(AUTHORIZATION) String customerBearer,
                                                               @Path(value = "id") String budgetId,
                                                               @Query(value = "start") Long start,
                                                               @Query(value = "end") Long end);
    ...
    @GET(BUDGETS_SUMMARIES)
    Mono<BudgetsSummariesResponseClientDto> findAllWithSummaries(@Header(AUTHORIZATION) String customerBearer,
                                                                 @Query(value = "includeArchived") Boolean includeArchived);
